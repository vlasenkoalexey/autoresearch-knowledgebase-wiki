...
2. **Read user invocation**: the user may have typed `/start-experiment <model> <lane>` (e.g. `/start-experiment afm_pt_moe tpu`). Parse if present.
3. **Ask**: if neither resolves both, use `AskUserQuestion` to ask. Offer the model folders under `wiki/experiments/` (currently: `afm_pt_moe_autoresearch_optimization`, `afmv7_autoresearch_optimization`, `gemma4_autoresearch_optimization`, `llama3_8B_autoresearch_optimization`); then ask for lane (typically `tpu`, `jax`, `torchax` — list whatever subdirectories of the chosen model folder actually contain a `program.md`).
...
```
Effective program for afm_pt_moe / tpu:
...
Examples: `afm_pt_moe` → `afm-pt-moe`, `gemma4` → `gemma4`, `llama3_8B` → `llama3-8b`.

Print both to the user before continuing:

```
USER_PREFIX = alekseyv  (from $USER segment)
MODEL_NAME  = afm-pt-moe  (auto-derived from folder)
...
v5p (2x2x2) cluster pool (4 candidates, 3 selected):
  ...
  ✗ wenxindong-pw-v5p-16-2     → occupied by my own jax-lane workload `alekseyv-afm-pt-moe-jax-v204-...`
...
1. RE-GROUND from disk (ORDER MATTERS):
   (a) PROGRAM (methodology — the drift-prevention anchor; do NOT skip):
       ...
       sections like "AFM-specific CAN additions".
...
2. WALK CLUSTER_POOL — for each cluster independently:
   ...
   (c) Else (cluster idle, ready for next dispatch). Walk program.md
       ...
       (v) VALIDATE length (XPK rejects ≥40 at parse time):
             ...
             Budget for AFM PT MoE JAX (alekseyv-afm-pt-moe-jax-v<NNN>- = 29
