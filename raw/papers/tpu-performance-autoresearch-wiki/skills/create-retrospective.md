...
Maps the search space already explored on a `(model, lane)` pair, identifies where coverage is incomplete, and ranks unexplored directions. Run this **before** declaring a lane exhausted — the 2026-05-21 case (afm-pt-moe-jax declared exhausted; tokamax-CE kernel +4.18 pp landed a week later) is exactly the failure this skill exists to prevent.
...
- **Manual invocation for human review** — `/create-retrospective afm_pt_moe jax` produces the file; user reads + decides.
...
Required:
- `<model>` `<lane>` — positional. Example: `/create-retrospective afm_pt_moe jax`.
...
Mode: INCREMENTAL (prior: 2026-05-25-afm-pt-moe-jax-retrospective.md; +N new experiments since)
...
- v418 (afm-pt-moe-tpu): tested torch_tpu.scan + register_autograd integration. Proposed new topic: **"Autograd-integrated structural rewrites"** — distinct from torch.compile + scan because the lever is operating BELOW Dynamo (C++/MLIR autograd hooks rather than compile-time graph capture).

- v391y (afm-pt-moe-jax): tested chunked-XLA backward kernel. Proposed new topic: **"Compiler-side loss kernel rewriting"** — adjacent to Pallas kernels but the lever is XLA-pass-level rather than custom kernel.
...
- **afm-pt-moe-tpu**: 3B/v5p-16 frontier 14.50% MFU @ v885; lbs=8 SimpleFSDP is the recent shift.
...
- **afm-pt-moe-torchax**: 3B/v5p-16 frontier 0.17% MFU (structural floor); no immediate transfer candidates.
- **afm-pt-moe-maxtext**: not yet ingested for this model.

### Sibling retrospective candidates (from sibling lanes' own analysis)

- **afm-pt-moe-tpu** retrospective (2026-05-28, recent):
...
- **afm-pt-moe-torchax** retrospective (2026-04-10, STALE — 50+ days old; low confidence):
...
```
Retrospective written: wiki/analyses/2026-06-02-afm-pt-moe-jax-retrospective.md
