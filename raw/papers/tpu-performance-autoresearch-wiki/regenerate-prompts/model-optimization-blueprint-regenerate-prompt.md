...
helps an agent decide:
  ...
  (e) WHICH LANE-SPECIFIC CAVEAT applies (JAX-stack vs torch_tpu vs
...
3. Phase 0 — substrate sanity (mandatory pre-flight). The
   ...
   audit; torch_tpu wheel -c opt; FSDP divisibility).
...
5. Phase 2 — known-good high-yield flag bundle. SPLIT by lane:
   Universal pre-flight, On JAX-stack (MaxText bundles + VMEM),
   On torch_tpu (much smaller bundle). Dedicated VMEM-flag-value
   sweep procedure (hyperparameter, 65k–131k range, per-gen cap).
...
6. Phase 3 — kernel + structural code work. SPLIT by lane into
   ...
   3E torchax-thin) and torch_tpu sub-ladder (3D compile firing,
   3B removals, 3C structural). Eight of the top-10 unitary
   experiment lifts live here.
...
11. Universal levers — confirmed multi-model. The 8 patterns that
    ...
    drop upcast, adamw_bf16_state, JAX compile cache, torch_tpu
    -c opt wheel).
12. Anti-playbook — top 5 traps that repeated. The refuted-pattern
    categories from the ledger, each with the trap concept page
    cross-link.
13. The compounding principle. Canonical walks (AFM PT MoE jax
    18→26.6%, AFM PT MoE tpu v2 0.18→14.2%, Llama 3 8B jax beats
    MaxText, DeepSeek V3 v5p-512 7→27%). Cite
    compounding-optimizations concept.
14. Lane-specific caveats. JAX, torchax, torch_tpu, MaxText —
    one short list per lane with what's different.
15. Where to look next — open research questions. The structural
    gaps the ledger has called out (e.g., Zig-Zag ring attention,
    torch_tpu C++/MLIR scan backward, TP=2 code path,
    MaxText Maverick EP-collective-permute port,
...
19. Sources — the corpora distilled (AFM PT MoE + AFMv7 + DeepSeek
    V3 + Gemma 4 + Llama 3 8B + Llama 4 + Flux + public-wiki
    mirrors) with counts. Then the substrate sources cited (link
    to the index's canonical reference stacks).
...
- LANE-SPLIT WHERE IT MATTERS. Phase 2 and Phase 3 ALWAYS have
  explicit "On JAX-stack" vs "On torch_tpu" subsections. Phase 0/1/
  4/5/6 are mostly shared. Cross-lane levers that flip sign
  (e.g., Pallas segment_matmul +19% on JAX, 4.4× LOSS on torch_tpu)
  must be called out at both subsections.
...
- COMPOUNDING WALKS ARE LIVING DATA. When a frontier moves on a
  cited variant (e.g., AFM PT MoE jax 3B/v5p-16 moves past
  26.6%), refresh the walk table; don't accumulate stale numbers.
...
8. Refresh Lane-specific caveats. Have any structural ceilings
   ...
   torch_tpu C++ collective coalescing path)?
...
- The TL;DR ladder has all 7 phases with effort + typical lift +
  citations.
- Every Phase 2 / Phase 3 section has BOTH a JAX-stack AND a
  torch_tpu subsection where the levers differ.
