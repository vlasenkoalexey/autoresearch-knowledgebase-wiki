...
5. wiki/concepts/ — read filenames + lead paragraphs. The relevant
   ...
   - libtpu: libtpu-flags
6. wiki/codebases/xprof-cli.md — THIS IS THE SINGLE SOURCE OF TRUTH
...
4. mcp__xprof__ tool catalog — full per-tool reference. Each row:
   tool name (with anchor ID), file path + line in raw/code/xprof-cli,
   data path (HTTP / .xplane.pb / XLA dump dir), 1-line use.
   Mirror exactly what wiki/codebases/xprof-cli.md says — don't
   re-derive.
...
2. Walk wiki/codebases/xprof-cli.md first. If the tool catalog has
...
- The tool catalog (§ 4) row count matches the count in
  wiki/codebases/xprof-cli.md (or differs only by tools intentionally
